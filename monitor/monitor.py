...
#  IDs: 0=kpi 1=kdi 2=kpv 3=kiv 4=kpo 5=kdo 6=vd 7=ramp 8=alphad 9=c1
...
_MOTOR_TEST_DEFS = [
    (12, "Prueba izq [V]", "Rueda izquierda: voltaje fijo (0 = control PID normal)"),
    (13, "Prueba der [V]", "Rueda derecha:   voltaje fijo (0 = control PID normal)"),
]
...
class GainsBar(QWidget):
    def __init__(self, send_fn):
        ...
        # ── Fila 1: ganancias ──────────────────────────────────────────────
        title1 = QLabel("GANANCIAS Y REFERENCIAS  —  Enter para enviar")
        title1.setStyleSheet(f"color:{C['muted']}; font-size:10px; font-weight:bold;")
        outer.addWidget(title1)

        row1 = QHBoxLayout()
        row1.setSpacing(6)
        self._fields = {}

        for gid, name, tip, default in _GAIN_DEFS:
            col = QVBoxLayout()
            col.setSpacing(1)
        ...
        btn_all = QPushButton("Enviar\ntodo")
        btn_all.setFixedSize(64, 42)
        btn_all.setToolTip("Envía todos los valores de ganancias (NO envía prueba motores)")
        btn_all.clicked.connect(self._send_all)
        row1.addWidget(btn_all, alignment=Qt.AlignmentFlag.AlignVCenter)
        ...
        sep.setStyleSheet(f"color:{C['bg3']};")
        outer.addWidget(sep)

        # ── Fila 2: prueba de motores ──────────────────────────────────────
        title2 = QLabel("PRUEBA MOTORES  —  voltaje fijo por rueda (0 V = regresa al PID)")
        title2.setStyleSheet(f"color:{C['orange']}; font-size:10px; font-weight:bold;")
        outer.addWidget(title2)

        row2 = QHBoxLayout()
        row2.setSpacing(10)

        for gid, name, tip in _MOTOR_TEST_DEFS:
            col = QVBoxLayout()
            col.setSpacing(1)

            lbl = QLabel(name)
            lbl.setStyleSheet(f"color:{C['orange']}; font-size:9px;")
            lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
            ...
            edit.setToolTip(tip)
            edit.setStyleSheet(
                f"background:{C['bg3']}; border:1px solid {C['orange']}44;"
                f" border-radius:5px; padding:4px 8px; color:{C['orange']};")
            edit.returnPressed.connect(lambda g=gid, e=edit: self._on_enter(g, e))

            btn_send = QPushButton("▶")
            btn_send.setFixedSize(28, 28)
            btn_send.setToolTip(f"Enviar {name}")
            btn_send.setStyleSheet(
                f"background:{C['orange']}; color:{C['bg']}; border:none;"
                f" border-radius:4px; padding:2px; font-weight:bold; font-size:11px;")
            btn_send.clicked.connect(lambda checked=False, g=gid, e=edit: self._on_enter(g, e))

            field_row = QHBoxLayout()
            field_row.setSpacing(3)
            field_row.addWidget(edit)
            field_row.addWidget(btn_send)
        ...
        btn_reset.setFixedSize(56, 42)
        btn_reset.setToolTip("Envía 0 V a ambas ruedas → regresa al control PID")
        btn_reset.setStyleSheet(
            f"background:{C['bg3']}; color:{C['orange']}; border:1px solid {C['orange']}66;"
            f" border-radius:5px; padding:5px 10px; font-weight:bold;")
        btn_reset.clicked.connect(self._reset_motors)
        row2.addWidget(btn_reset, alignment=Qt.AlignmentFlag.AlignVCenter)
    ...
    def _send_all(self):
        for gid, edit in self._fields.items():
            if gid in (12, 13):
                continue  # no envía prueba de motores con "enviar todo"
            try:
                self._send(gid, float(edit.text()))
            except ValueError:
                pass
    ...
    def _reset_motors(self):
        for gid in (12, 13):
            self._send(gid, 0.0)
            if gid in self._fields:
                self._fields[gid].setText("0.0000")
...
class MainWindow(QMainWindow):
    ...
    def _send_gain(self, gain_id: int, value: float):
        if self._esp32_ip == "—":
            self._on_status(
                "⚠  Sin IP del ESP32 — pulsa 'Escuchar' y espera al menos un paquete de telemetria", True)
            self._botbar.setText("  ⚠  ESP32 no detectado — espera un paquete de telemetria antes de enviar")
            self._botbar.setStyleSheet(
                f"background:{C['bg2']}; color:{C['red']}; font-size:10px; font-weight:bold;"
                f" border-top:1px solid {C['bg3']}; padding-left:8px;")
            return
        ...
        try:
            ...
            names = ["kpi","kdi","kpv","kiv","kpo","kdo","vd","ramp","α_ref","c1",
                     "calpha","curva","test_izq","test_der"]
